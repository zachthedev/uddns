...
/** An error response from a batch that failed part way, which carries what it did. */
type BatchFailureBody = ErrorResponseBody & { data: NonNullable<ErrorResponseBody['data']> };

/** The record list call the worker makes, which always names the record. */
type RecordListCall = RecordListParams & Required<Pick<RecordListParams, 'name'>>;
...
describe('Worker fetch handler', () => {
  ...
  describe('Request logging', () => {
    ...
    it('logs the path and puts no query string on any console sink', async () => {
      ...
      mockCloudflareClient.user.tokens.verify.mockResolvedValue({ id: 'tid', status: 'active' });
      ...
      const emitted = JSON.stringify(
        [...logSpy.mock.calls, ...errorSpy.mock.calls, ...warnSpy.mock.calls].map((args) =>
          args.map((arg: unknown) => (arg instanceof Error ? { message: arg.message, stack: arg.stack } : arg)),
        ),
      );
  ...
  describe('Rate limiter', () => {
    ...
    it('allows the request when RATE_LIMITER returns success:true', async () => {
      const rateLimiterMock = rateLimiterOf(env);
  ...
  describe('Auth parsing', () => {
    it('accepts Basic credentials and constructs SDK with apiToken only (no apiEmail)', async () => {
      ...
      expect(vi.mocked(Cloudflare)).not.toHaveBeenCalledWith(
        expect.objectContaining({ apiEmail: expect.anything() as unknown }),
      );
    });
  ...
  describe('Access gate', () => {
    it('verifies the token before touching KV when no ACCESS_KEY is set', async () => {
      ...
      kvOf(env).get.mockImplementation(() => {
        order.push('kv');
        return Promise.resolve(null);
      });

      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=a.example.com,b.example.com',
        {
          headers: { Authorization: createAuthHeader('user', 'token') },
        },
      );
    ...
    it('does not serve one token a cache entry another token wrote', async () => {
      ...
      const storage = new Map([
        [victimKey, JSON.stringify({ ip: '203.0.113.77', updatedAt: '2024-01-01T00:00:00.000Z' })],
      ]);
      kvOf(env).get.mockImplementation((key: string) => Promise.resolve(storage.get(key) ?? null));
      mockCloudflareClient.user.tokens.verify.mockResolvedValue({ id: 'attacker-token', status: 'active' });
      mockCloudflareClient.zones.list.mockReturnValue(mockPage({ result: [] }));
      ...
      expect(response.status).toBe(400);
      expect(kvOf(env).get).toHaveBeenCalledWith('ip:attacker-token:vpn.example.com:A');
      expect(kvOf(env).get).not.toHaveBeenCalledWith(victimKey);
    });
  ...
  describe('DNS record construction', () => {
    ...
    it('ip4=auto when client connected over IPv6 with ip6=auto also set produces only AAAA records', async () => {
      ...
      kvOf(env).get.mockResolvedValue(null);
      kvOf(env).put.mockResolvedValue(undefined);
      ...
      const response = await worker.fetch(request, env, ctx);
      const body = await response.json<UpdateResponseBody>();
    ...
    it('rejects ip6 value that does not contain a colon', async () => {
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&ip6=notanipv6&hostnames=test.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it.each([
      ...
      expect(kvOf(env).get).not.toHaveBeenCalled();
    });
    ...
    it.each([
      ...
      const request = createMockRequest(`https://example.com/update?${param}&hostnames=test.example.com`, {
        headers: validAuth,
      });
    ...
    it.each([
      ...
      const request = createMockRequest(
        `https://example.com/update?ip6=${encodeURIComponent(value)}&hostnames=test.example.com`,
        {
          headers: validAuth,
        },
      );
    ...
    it.each([
      ...
      const request = createMockRequest(
        `https://example.com/update?${param}=${encodeURIComponent(value)}&hostnames=test.example.com`,
        {
          headers: validAuth,
        },
      );
    ...
    it.each([
      ...
      mockCloudflareClient.dns.records.list.mockReturnValue(
        mockPage({
          result: [
            { id: 'r1', name: 'test.example.com', type: 'AAAA', content: '2001:db8::9', proxied: false, ttl: 1 },
          ],
        }),
      );
      mockCloudflareClient.dns.records.update.mockResolvedValue(undefined);
      ...
      const request = createMockRequest(
        `https://example.com/update?ip6=${encodeURIComponent(sent)}&hostnames=test.example.com`,
        {
          headers: validAuth,
        },
      );
      ...
      expect(response.status).toBe(200);
      expect(mockCloudflareClient.dns.records.update).toHaveBeenCalledWith(
        'r1',
    ...
    it('treats a differently spelled but identical address as no change', async () => {
      ...
      mockCloudflareClient.dns.records.list.mockReturnValue(
        mockPage({
          result: [
            { id: 'r1', name: 'test.example.com', type: 'AAAA', content: '2001:db8::1', proxied: false, ttl: 1 },
          ],
        }),
      );
    ...
    it.each([
      ...
      const request = createMockRequest(
        `https://example.com/update?ip4=1.2.3.4&hostnames=${encodeURIComponent(hostname)}`,
        {
          headers: validAuth,
        },
      );
    ...
    it.each([
      ...
      const request = createMockRequest(
        `https://example.com/update?ip4=1.2.3.4&hostnames=a.example.com&zone=${encoded}`,
        {
          headers: validAuth,
        },
      );
    ...
    it('pins the content type so nothing sniffs a caller-supplied value', async () => {
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=a.example.com&zone=under_score.example',
        {
          headers: validAuth,
        },
      );
    ...
    it('handles multiple hostnames separated by commas', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=test1.example.com,test2.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('adds an AAAA record per hostname when ip6 is provided alongside ip4', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&ip6=2001:db8::1&hostnames=h1.example.com,h2.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('rejects ntfy param that is not a valid URL', async () => {
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=test.example.com&ntfy=not-a-url',
        {
          headers: validAuth,
        },
      );
  ...
  describe('Token verification', () => {
    ...
    it('rejects an inactive token', async () => {
      mockCloudflareClient.user.tokens.verify.mockResolvedValue({ id: 'tid', status: 'expired' });
    ...
    it.each([
      ...
      mockCloudflareClient.user.tokens.verify.mockResolvedValue(verification);
  ...
  describe('KV cache', () => {
    ...
    it('uses cache key ip:<tokenId>:<hostname>:<type>', async () => {
      ...
      expect(kvMock.put).toHaveBeenCalledWith('ip:tid:test.example.com:A', expect.any(String), {
    ...
    it('handles partial cache: cached hostname skipped, pending hostname fetches DNS', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=h1.example.com,h2.example.com',
        {
          headers: validAuth,
        },
      );
      ...
      const cachedEntry = body.data.records.find((r) => r.hostname === 'h1.example.com');
  ...
  describe('Zones cache', () => {
    ...
    beforeEach(() => {
      mockCloudflareClient.user.tokens.verify.mockResolvedValue({ id: 'token-id-123', status: 'active' });
    });
    ...
    it.each([
      ...
      kvMock.get.mockImplementation((key: string) =>
        Promise.resolve(key.startsWith('zones:') ? JSON.stringify(cached) : null),
      );
      kvMock.put.mockResolvedValue(undefined);
    ...
    it('stops walking zone pages at the ceiling instead of spending unbounded subrequests', async () => {
      ...
      const zones = Array.from({ length: 1200 }, (_, i) => ({
        id: `zone${String(i)}`,
        name: `z${String(i)}.example.net`,
      }));
      mockCloudflareClient.zones.list.mockReturnValue(
        mockPage({ result: zones }, { result: [{ id: 'late', name: 'example.com' }] }),
      );
    ...
    it('skips zones.list when zones cache is a hit and uses the cached zone for record lookup', async () => {
      ...
      expect(mockCloudflareClient.dns.records.list).toHaveBeenCalledWith(
        expect.objectContaining({ zone_id: 'zone-from-cache' }),
      );
    });
  ...
  describe('Refusal counter', () => {
    ...
    const refusalCalls = (): string[][] => {
      ...
      const stub = refusalsOf(env).getByName('token-id-123');
    ...
    it('counts every refusal in a batch however the caller orders it', async () => {
      ...
      expect(refusalsOf(env).getByName).toHaveBeenCalledWith('token-id-123');
    });
    ...
    it('loses nothing when a burst is followed by silence', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const request = createMockRequest(
          'https://example.com/update?ip4=1.2.3.4&hostnames=a.elsewhere.com,b.elsewhere.com',
          {
            headers: validAuth,
          },
        );
    ...
    it('reads back what it counted', async () => {
      ...
      const update = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=a.elsewhere.com,b.elsewhere.com',
        {
          headers: validAuth,
        },
      ...
      const body = await (await worker.fetch(history, env, ctx)).json<HistoryResponseBody>();
    ...
    it('counts a zone filter naming a zone the token cannot see', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=a.example.com&zone=other.example',
        {
          headers: validAuth,
        },
      );
    ...
    it('caches an empty zone list only briefly', async () => {
      ...
      expect(kvOf(env).put).toHaveBeenCalledWith('zones:token-id-123', '[]', {
        expirationTtl: 60,
      });
      ...
      await expect(kvOf(env).get('zones:token-id-123')).resolves.toBe('[]');
    });
    ...
    it('logs once when a token crosses the refusal threshold', async () => {
      ...
      for (let round = 0; round < 4; round++) {
        const hostnames = Array.from(
          { length: 40 },
          (_unused, i) => `r${String(round)}h${String(i)}.elsewhere.com`,
        ).join(',');
        const request = createMockRequest(`https://example.com/update?ip4=1.2.3.4&hostnames=${hostnames}`, {
          headers: validAuth,
        });
        await worker.fetch(request, env, ctx);
        await settle();
    ...
    it('never crosses the threshold on one hostname however long it is retried', async () => {
      ...
      for (let poll = 0; poll < 200; poll++) {
        const request = createMockRequest('https://example.com/update?ip4=1.2.3.4&hostnames=typo.elsewhere.com', {
          headers: validAuth,
        });
        await worker.fetch(request, env, ctx);
      }
    ...
    it('does not count a permission failure from the Cloudflare API', async () => {
      ...
      mockCloudflareClient.dns.records.list.mockImplementation(() => ({
        then: (_resolve: unknown, reject: (reason: unknown) => void) => {
          reject(new MockPermissionDeniedError());
        },
      }));
    ...
    it('answers a revoked token with 401 and does not count it', async () => {
      ...
      mockCloudflareClient.dns.records.list.mockImplementation(() => ({
        then: (_resolve: unknown, reject: (reason: unknown) => void) => {
          reject(new MockAuthenticationError());
        },
      }));
    ...
    it('does not count a transport failure against the caller', async () => {
      ...
      mockCloudflareClient.dns.records.list.mockImplementation(() => ({
        then: (_resolve: unknown, reject: (reason: unknown) => void) => {
          reject(new MockInternalServerError());
        },
      }));
    ...
    it('keeps the cached zone list when a hostname is refused', async () => {
      ...
      expect(kvOf(env).delete).not.toHaveBeenCalledWith('zones:token-id-123');
    });
    ...
    it('does not count a token whose zone list hit the ceiling', async () => {
      ...
      vi.spyOn(console, 'warn').mockImplementation(() => undefined);
      const zones = Array.from({ length: 1200 }, (_unused, i) => ({
        id: `zone${String(i)}`,
        name: `z${String(i)}.example.net`,
      }));
    ...
    it('counts a dual-stack hostname once, not once per family', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&ip6=2001:db8::1&hostnames=a.elsewhere.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('does not count a name the token holds but the caller scoped out', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=b.other.example&zone=example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('does not fail the request when the counter is unreachable', async () => {
      refusalsOf(env).getByName.mockImplementation(() => {
    ...
    it('reports zero rather than failing when the counter cannot be read', async () => {
      ...
      const response = await worker.fetch(request, env, ctx);
      const body = await response.json<HistoryResponseBody>();
  ...
  describe('Zone and record matching', () => {
    ...
    it('queries only the zone that could host the hostname and updates there', async () => {
      ...
      expect(mockCloudflareClient.dns.records.update).toHaveBeenCalledWith(
        'record1',
        expect.objectContaining({ zone_id: 'zone2' }),
      );
    });

    it('does not scale the record lookup with the number of zones on the token', async () => {
      const manyZones = Array.from({ length: 60 }, (_, i) => ({
        id: `zone${String(i)}`,
        name: `z${String(i)}.example.net`,
      }));
      mockCloudflareClient.zones.list.mockReturnValue(
        mockPage({ result: [...manyZones, { id: 'target', name: 'test.com' }] }),
      );
      mockCloudflareClient.dns.records.list.mockReturnValue(
    ...
    it('filters zones by name when zone param is provided', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=test.example.com&zone=example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('returns 400 when zone filter matches no available zones', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=test.example.com&zone=notmine.com',
        {
          headers: validAuth,
        },
      );
  ...
  describe('Record update flow', () => {
    ...
    it('does not call dns.records.update when existing content already matches target IP (DNS-delta no-op)', async () => {
      ...
      expect(kvOf(env).put).toHaveBeenCalled();
    ...
    it('successfully updates multiple DNS records and sends a grouped notification', async () => {
      ...
      mockCloudflareClient.dns.records.list
        .mockReturnValueOnce(
          mockPage({
            result: [
              { id: 'record1', name: 'test1.example.com', type: 'A', content: '1.2.3.4', proxied: false, ttl: 1 },
            ],
          }),
        )
        .mockReturnValueOnce(
          mockPage({
            result: [
              { id: 'record2', name: 'test2.example.com', type: 'A', content: '1.2.3.4', proxied: true, ttl: 300 },
            ],
          }),
        );
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.5&hostnames=test1.example.com,test2.example.com',
        {
          headers: validAuth,
        },
      );
  ...
  describe('Audit events', () => {
    ...
    it('audits and notifies the records that landed even when a sibling record fails', async () => {
      ...
      mockCloudflareClient.dns.records.list.mockImplementation((params: RecordListCall) =>
        params.name.exact === 'good.example.com'
          ? mockPage({
              result: [{ id: 'r1', name: 'good.example.com', type: 'A', content: '1.2.3.0', proxied: false, ttl: 1 }],
            })
          : mockPage({ result: [] }),
      );
      mockCloudflareClient.dns.records.update.mockResolvedValue(undefined);
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=good.example.com,missing.example.com',
        {
      ...
      expect(auditDbMock.statement.bind).toHaveBeenCalledWith(
    ...
    it('reports the records that landed in the error body', async () => {
      ...
      const response = await worker.fetch(request, env, ctx);
      const body = await response.json<BatchFailureBody>();
    ...
    it('surfaces the actionable refusal rather than a transient failure on an earlier record', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=aaa.example.com,zzz.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it.each([
      ...
      mockCloudflareClient.dns.records.list.mockImplementation((params: RecordListCall) => ({
        then: (_resolve: unknown, reject: (reason: unknown) => void) => {
          reject(params.name.exact === 'first.example.com' ? new First() : new Second());
        },
      }));

      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=first.example.com,second.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('logs every failure in a batch, not only the one it reports', async () => {
      ...
      const request = createMockRequest(
        'https://example.com/update?ip4=1.2.3.4&hostnames=a.example.com,b.example.com,c.example.com',
        {
          headers: validAuth,
        },
      );
    ...
    it('writes no audit batch when every record in the request fails', async () => {
      ...
      expect(auditDbOf(env).batch).not.toHaveBeenCalled();
    });
  ...
  describe('GET /history endpoint', () => {
    ...
    it.each([
      ...
      expect(response.status).toBe(200);
      const bindArgs = lastBind(auditDbOf(env).statement);
    ...
    it('carries the cursor and the tally on the first page', async () => {
      ...
      auditDbMock.statement.all.mockResolvedValue({
        results: Array.from({ length: 3 }, (_unused, i) => ({
          id: i + 1,
          occurred_at: `2026-08-06T00:00:00.00${String(i)}Z`,
        })),
      });

      const request = createMockRequest('https://example.com/history?limit=2', { headers: validAuth });
      const body = await (await worker.fetch(request, env, ctx)).json<HistoryResponseBody>();
    ...
    it('omits the tally on a continuation page', async () => {
      ...
      const request = createMockRequest('https://example.com/history?before=2026-08-06T00%3A00%3A01.000Z%7C2', {
        headers: validAuth,
      });
      const body = await (await worker.fetch(request, env, ctx)).json<HistoryResponseBody>();

      expect(body.data.refusedToday).toBeNull();
      expect(refusalsOf(env).getByName).not.toHaveBeenCalled();
    });
  ...
  describe('Full update flow', () => {
    it('completes full pipeline with IP change detection, DNS update, KV storage, and notification', async () => {
      ...
      expect(mockCloudflareClient.dns.records.update).toHaveBeenCalled();
      expect(kvMock.put).toHaveBeenCalledWith('ip:token-id-123:test.example.com:A', expect.any(String), {
        expirationTtl: 2592000,
      });
      expect(waitUntilMock).toHaveBeenCalled();
    ...
    it('ip4 + ip6 both literal: 2 hostnames produce 4 records in the response', async () => {
      ...
      expect(response.status).toBe(200);
      expect(body.data.records).toHaveLength(4);
      const types = body.data.records.map((r) => r.type);
