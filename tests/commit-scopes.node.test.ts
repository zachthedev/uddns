...
const documents = new Map<string, string>(
	[...markdownIn('', false), ...markdownIn('docs/', false), ...markdownIn('.github/', true)].map((path) => [path, read(path)]),
);
...
describe('the contributor guide', () => {
	...
	it('names only scripts that package.json defines', () => {
		...
		expect(named.length, 'CONTRIBUTING.md names no `bun run` script, so this binding asserts nothing').toBeGreaterThan(0);
		expect(
