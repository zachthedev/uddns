...
- Install: `bun install`
- Verify everything: `bun run check`. It is the whole gate, and every workflow calls a script rather
  than listing its steps. `bun run check:rows` prints its rows, cheapest first. A new check is a row in
  `scripts/check.ts`, not a workflow step.
...
- `bun run audit` is exactly the kind of check that is not hermetic, so it runs on pull requests and on
  pushes to main through `ci.yml`, daily through `audit.yml`, and never on the deploy path.
...
- Known exposure, because nothing reports it: the scheduled workflows depend on Renovate staying alive.
  ...
  `audit.yml` go quiet together and are then disabled, with no notification. The thing that
  would report the failure is the thing that failed, so check Renovate is still opening pull requests.
- CI runs the gate on Linux, Windows and macOS in the `gate` job. `deploy.yml` and `release-please.yml`
  run it again, because a manual deploy takes any ref, and a tag and a GitHub Release cannot be withdrawn
  once published.
- The lefthook `pre-push` hook runs `check:quick`, the gate without its test row, and refuses the push
  when it fails. The test row stays out of the hook because the Durable Object and D1 suites fail under a
  long temp path on Windows, and a hook that cannot pass on a maintainer's machine gets bypassed. CI's
  `gate` job runs the whole of `check`.
- One row of the gate is not hermetic by choice: in `check`, `zizmor` runs online when a GitHub token is at
  hand (`GH_TOKEN`, else `gh auth token`), so its known-vulnerable-actions and stale-ref audits read GitHub.
  `check:quick` runs it offline, so the push hook needs no network and no token, and `ZIZMOR_OFFLINE=true`
  forces offline for `check`. The token reaches zizmor's process alone; the gate takes `GH_TOKEN` out of the
  environment every other row inherits.
- Checks that run in CI and not in the gate, each with the reason it sits outside. A working machine
  ...
  commits. The `Commit Messages` job lints the pull request's own commit range and its title, neither of
  which exists before the pull request does. The trufflehog image is pinned by digest and pulled from
  ghcr.io, which is content-addressed, so no step compares that digest against a second registry. The
  deploy dry run (`bun x wrangler deploy --dry-run`: offline, no account or token, because it marks an
  absent resource ID as inherited rather than resolving it) is a gate candidate still outside the gate. The
  audit is none of these. It sits outside the gate by scope rather than as an exception, the same way a
